...
import sys
import platform
from metrics import request_duration_histogram, cart_addition_total, errors_total, cpu_usage_gauge, active_sessions_gauge, update_cpu_usage, update_active_sessions
...
def add_to_cart(product_id):
    ...
    try:
        ...
        cart_addition_total.labels(product_id=product_id).inc(quantity)
        ...
        flash('Ocorreu um erro ao adicionar o item ao carrinho.', 'danger')
        return redirect(url_for('shop'))
...
def index():
    ...
    import random
    time.sleep(random.uniform(0.00, 2))
...
def metrics():
    try:
        update_active_sessions()       # Atualiza sessões ativas
        update_cpu_usage()             # Atualiza CPU SOB DEMANDA (não em thread)
        print("📊 Métricas Gauge atualizadas para coleta do Prometheus")
    except Exception as e:
